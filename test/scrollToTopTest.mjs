...
describe("Scroll to top Button", async () => {
  ...
  const scrollDown = async (driver) => {
    ...
    let waitTime = 2000;
    while (
      (await driver.executeScript("return window.scrollY;")) < deltaY &&
      waitTime > 0
    ) {
  ...
  const scrollToTop = async (driver) => {
    ...
    while (
      (await driver.executeScript("return window.scrollY;")) > 0 &&
      waitTime > 0
    ) {
      await driver.manage().setTimeouts({ implicit: 200 });
      waitTime -= 200;
  ...
  context("For not logged in users", async () => {
    ...
    it("disappears after manually scrolling to top", async () => {
      await driver.get("https://katalon-demo-cura.herokuapp.com/profile.php");
      const toTopButton = await driver.findElement(
        By.xpath('//*[@id="to-top"]')
      );
  ...
  context("For logged in users", async () => {
    before(async () => {
